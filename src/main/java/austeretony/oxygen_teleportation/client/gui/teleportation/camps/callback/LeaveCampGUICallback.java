...
public class LeaveCampGUICallback extends AbstractGUICallback {
    ...
    public LeaveCampGUICallback(TeleportationMenuGUIScreen screen, CampsGUISection section, int width, int height) {
    ...
    @Override
    public void init() {
        this.addElement(new LeaveCampCallbackGUIFiller(0, 0, this.getWidth(), this.getHeight()));
        this.addElement(new GUITextLabel(2, 2).setDisplayText(ClientReference.localize("teleportation.gui.menu.leaveCampCallback"), true, GUISettings.instance().getTitleScale()));
    ...
    protected void onOpen() {
        this.requestLabel.setDisplayText(ClientReference.localize("teleportation.gui.menu.leaveCallback.request", this.section.getCurrentPoint().getName()), true, GUISettings.instance().getTextScale());
    }
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (element == this.cancelButton)
            ...
            TeleportationManagerClient.instance().getCampsManager().leaveCampPointSynced(this.section.getCurrentPoint().getId());
