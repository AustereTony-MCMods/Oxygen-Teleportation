...
public class LocationCreationCallback extends AbstractGUICallback {
    ...
    public LocationCreationCallback(TeleportationMenuScreen screen, LocationsSection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new OxygenTextLabel(4, 12, ClientReference.localize("oxygen_teleportation.gui.menu.callback.locationCreation"), EnumBaseGUISetting.TEXT_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
    ...
    private void confirm(boolean mouseClick) {
        if (mouseClick || (!this.nameField.isDragged()
            ...
            TeleportationManagerClient.instance().getLocationsManager().createLocationPointSynced(this.nameField.getTypedText(), this.descriptionBoxField.getTypedText());
            this.close();
        }
    }
